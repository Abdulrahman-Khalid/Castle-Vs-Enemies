...
private:
	...
	color
		CastleColor ,
		LineColor ,
		MsgColor ,		//Messages color
		BkGrndColor ,	//Background color
		StatusBarColor ;
...
public:
	...
	// Output Functions  ---------------------------
	void PrintMessage(string msg, int region = -1 , int br = 0) const;
	...
	void DrawCastle(Castle* C) const;	    // draws the castle with the towers and 4 regions
	void DrawStatusBarBoxes() const;
	void DrawEnemies(Enemy* enemies[], int start, int size,int) const;
	...
	void DrawUnpavedArea(int region_num,int unpaved_dist) const;
};
